...
In a terminal, run the following `curl` commands to create one `debezium` and two `elasticsearch-sink` connectors on `kafka-connect`
```
...
2. Fix the UPPERCASE field names that `KSQL` is generating. It is possible using double-quotes but, tried and it breaks the `CREATE STREAM`. `Allow statement to specify the casing (camel case, uppercase, etc) for field names when serialized to output topic #1039` https://github.com/confluentinc/ksql/issues/1039

3. Replace the deprecated `topic.index.map` configured in `elasticsearch-sink-*` connectors. Waiting for those `kafka-connect-elasticsearch` issues to be fixed:
